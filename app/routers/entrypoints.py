...
def store_user(request: Request, user: User):
    ...
    email_status= authentication_email(to_email=user.email, subject="Please confirm your email", redirect_url=SETTINGS.allowed_origins[0])
...
def forgot_password(request: Request, user: EmailRequest):
    ...
    if reset_password_email(to_email=user.email, subject="Reset your password", redirect_url=SETTINGS.redirect_url) == False:
        LOGGER.warning("password_change_failed", user=user.email, ip=client_ip)
